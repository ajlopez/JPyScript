...
var jpyparser = (function() {
    ...
    function ForInCommand(name, expr, cmd) {
        this.compile = function (options) {
            ...
            if (options.withvars)
                code += compileVars(this, options.names, options.othernames);
        ...
        this.collectVarNames = function (names, othernames) {
            addName(name, names, othernames);
            if (cmd.collectVarNames)
                cmd.collectVarNames(names, othernames);
        }
    }
    ...
    function AssignmentCommand(left, expr, oper) {
        ...
        this.collectVarNames = function (names, othernames) {
            if (left.getName)
                addName(left.getName(), names, othernames);
        }
    }
    ...
    function IfCommand(cond, thencmd, elsecmd) {
        this.compile = function (options) {
            ...
            code += 'if (' + cond.compile() + ') { ' + thencmd.compile() + ' }';
        ...
        this.collectVarNames = function (names, othernames) {
            if (thencmd.collectVarNames)
                thencmd.collectVarNames(names, othernames);
            if (elsecmd && elsecmd.collectVarNames)
                elsecmd.collectVarNames(names, othernames);
        }
    }
    ...
    function WhileCommand(cond, body) {
        ...
        this.collectVarNames = function (names, othernames) {
            if (body.collectVarNames)
                body.collectVarNames(names, othernames);
        }
    }
    ...
    function DefCommand(name, args, body) {
        ...
        this.compile = function() {
            ...
            code += ') { ' + body.compile({ withvars: true, names: [], othernames: args.slice() }) + ' }';
    ...
    function ClassCommand(name, body) {
        this.compile = function() {
            ...
            code += ') { ' + body.compile({ withvars: true }) + ' }';
    ...
    function CompositeCommand(cmds) {
        ...
        this.collectVarNames = function (names, othernames) {
            for (var k = 0; k < n; k++)
                if (cmds[k].collectVarNames)
                    cmds[k].collectVarNames(names, othernames);
        }
    }
    
    function addName(name, names, othernames) {
        if (othernames.indexOf(name) >= 0)
            return;
    ...
    function compileVars(cmd, names, othernames) {
        if (!cmd.collectVarNames)
            return '';
        ...
        names.forEach(function(name) { code += 'var ' + name + '; ' });
