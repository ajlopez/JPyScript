...
var jpyparser = (function() {
    ...
    function AssertCommand(expr) {
        this.compile = function (options) {
            options = options || { };
            var code = "if (__debug__ && !(" + expr.compile() + ")) { throw 'assert error' }";
            return code;
        }
    }
    ...
    function ClassCommand(name, body) {
        this.compile = function() {
            ...
            var options = { withvars: true, names: { } };
            
            code += body.compile(options);
            
            if (body.collectNames)
                body.collectNames(options.names);
                
            code += compileSelfDefs(options.names);
            
            if (!options.names || !options.names.defs || options.names.defs.indexOf('__init__') < 0)
